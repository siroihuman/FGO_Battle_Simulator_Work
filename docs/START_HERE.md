...
- フェーズ: 6／初期敵データ
- 状態: 共通戦闘・効果・ログ基盤、初期サーヴァント2騎、初期魔術礼装2着、初期概念礼装2枚の具体データと効果、完全戦闘ループ、保存・再開を実装済み。初期敵は黎明の炎腕（剣）1種と、種火集め（剣基準）極級の3 Wave・7個体に仕様確定済み。次は具体データと共通行動接続を実装する。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. `radiant-arm-of-dawn-saber`（黎明の炎腕・剣）の形式1敵データ、登録時検査、レジストリと、`ember-gathering-saber-extreme`の3 Wave・7個体を実装する（敵実装担当、推奨モデル: `gpt-5.6-sol`）。通常攻撃クリティカル10%、JSON化可能なランダム単体対象、敵ターン終了時チャージ1増加、業火、保存・再開・固定シードリプレイを共通基盤へ接続し、固有`mechanics`は作らない。
