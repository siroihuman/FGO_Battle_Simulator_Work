...
- フェーズ: 5／初期サーヴァント
- 状態: 共通戦闘・効果・ログ基盤、初期サーヴァント2騎、完全戦闘ループ、保存・再開、礼装選択・開始時適用、初期魔術礼装6スキルを実装済み。初期概念礼装2枚の対象・最大解放・固定Lv仕様を確定済み。次は2枚の具体データと効果を実装する。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. カレイドスコープと黒の聖杯の最大解放・Lv100データ、能力値、具体効果を実装する（概念礼装実装担当、推奨モデル: `gpt-5.6-sol`）。黒の聖杯の毎ターンHP減少の生存規則は追加資料で確認し、未確認のまま近似しない。
