...
- フェーズ: 7／初期戦闘UI（最小版完了）
- 状態: 登録済み初期サーヴァント・魔術礼装・概念礼装・極級敵設定から、個体別Lv／宝具Lv／礼装、固定シードを選択し、開始時ロードアウト初期化、初回配布、対象・3枚カード操作、状態・確定ログ表示、中断保存JSON・直接再開を`BattleSession`へ接続済み。UIは能力値・効果・対象・クリティカル・チャージを再計算しない。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. 既存のサーヴァント保有スキルと魔術礼装スキルを味方入力境界で操作できるよう、サーヴァントスキル操作を`BattleSession`のJSON操作履歴・中断保存・検証リプレイへ追加し、対象選択・CT・成立／不成立結果を最小UIへ接続する（UI／システム実装担当、推奨モデル: `gpt-5.6-sol`）。既存`resolveAllySkillUse`と`resolveBattleSessionMysticCodeSkill`を再利用し、UIで対象・効果・CTを再計算しない。
