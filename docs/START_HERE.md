...
- 状態: No.056「アグリッパ」と絆礼装「船嘴の黄金冠」、No.105「李広」と絆礼装「桃李の下の蹊」、No.107「サルバドール・ダリ」と絆礼装「死してなお」、共有スキル・状態アイコン素材は`main`へ統合済み。戦闘状況とリザルトから、開始時スナップショットを使う同一シードのやり直しと、新しい具体シードを使う別シードのやり直しを追加した。自動検査後、ユーザー環境の実画面受入とPR統合を待つ。BattleSession、中断保存形式4・データ1.38.0、直接再開、固定シードリプレイ、カード再配布、6乱数列は維持している。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. 戦闘中とリザルトで「同じシードで戦闘をやり直す」「違うシードで戦闘をやり直す」を確認する。同じシードでは初回手札を含めて同じ開始状態になること、別シードでは表示シードが変わり初期状態から始まることを確認する。合格ならPRを`main`へ統合する。
