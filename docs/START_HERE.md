...
- 状態: 登録済み初期データの設定、能動スキル、カード操作、極級3 Wave、勝利・敗北、入力／完全ターンログ、中断保存形式4の直接再開、旧形式3移行、固定シード再現を自動検査と390×844pxの実ブラウザーで確認済み。受入不具合は0件で、UIによる戦闘計算の重複もない。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. コマンドシャッフル等の初期対象外カード再配布効果について、発動可能な入力境界、現在手札・残りデッキ・配布周期、カード用乱数消費、カード別スター、宝具カード候補、ログ、保存・再開・固定シードリプレイの仕様を確定する（システム仕様担当、推奨モデル: `gpt-5.6-sol`）。この作業では仕様策定だけを行い、具体実装へ進まない。
