...
- フェーズ: 14／魔術協会制服（統合受入完了）
- 状態: 自動検査とChromium 149・390×844pxの実操作で、形式2登録、3スキル、対象・CT、カード再配布、入力ログ、中断保存形式4・データ1.38.0、直接再開、固定シードリプレイ、最小UIを合格と判定。不具合は0件。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. 延焼・蝕毒・呪厄によるスリップダメージ倍加について、対象状態、状態カテゴリ、重複、計算・丸め、発動順、解除・退場、保存・再開・リプレイ、ログ、UI接続を仕様として確定する（システム仕様策定担当、推奨モデル: `gpt-5.6-sol`、思考レベル: `high`）。資料で確定できない挙動は質問して停止し、具体実装やコンテンツ追加へ進まない。
