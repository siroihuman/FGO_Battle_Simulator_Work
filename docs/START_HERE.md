...
- 状態: No.105「李広」と絆礼装「桃李の下の蹊」、No.107「サルバドール・ダリ」と絆礼装「死してなお」、共有スキル・状態アイコン素材は`main`へ統合済み。No.056「アグリッパ」のスキル2・3で〔初代ローマ皇帝〕ではない味方を選ぶと不発になる問題を修正し、条件付き追加効果だけを対象なしとして基本効果を成立させる。自動検査後、ユーザー環境の実画面受入とPR統合を待つ。BattleSession、中断保存形式4・データ1.38.0、直接再開、固定シードリプレイ、カード再配布、6乱数列は維持している。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. アグリッパのユーザー環境の実画面確認を行い、3スキル、〔初代ローマ皇帝〕への追加強化、〔対人〕特防、Arts全体宝具、絆礼装「船嘴の黄金冠」を確認する。合格ならPRを`main`へ統合する。
