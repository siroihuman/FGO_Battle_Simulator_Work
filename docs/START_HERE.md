...
- 状態: 指定済み8騎の絆礼装を星4最大解放Lv80・ATK100・HP100で登録した。絆礼装は厳密なサーヴァント`dataId`だけに表示・装備でき、前衛限定の味方効果は交代・補充後もエンジン確定状態で有効／無効を更新する。自動検査を完了し、ユーザー環境の実画面受入とPR統合を待つ。BattleSession、中断保存形式4・データ1.38.0、直接再開、固定シードリプレイ、カード再配布、6乱数列は変更していない。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. 絆礼装のユーザー環境の実画面確認を行い、対象外サーヴァントで非表示、対象サーヴァントで表示、前衛限定効果の交代・補充後の切替を確認する。合格ならPRを`main`へ統合する。
