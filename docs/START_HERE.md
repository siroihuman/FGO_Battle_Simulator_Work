...
- 状態: No.007「本多忠勝」の具体データと、Quick撃破後も同Artsを同じ対象へ完遂してから退場・再選択する連続通常攻撃修正は実画面合格済み。連続攻撃中であることを明確にするため、同じ戦闘個体が戦闘不能対象へ攻撃している間はエンジン確定状態の敵カードとHP0を表示し続け、資源増減0でも「連続攻撃中」フレームを表示するよう追補した。BattleSession、中断保存形式4・データ1.38.0、直接再開、固定シードリプレイ、カード再配布、6乱数列は変更していない。HP0継続表示は自動検査完了・実画面再受入待ちである。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. ユーザー環境で本多忠勝Quick→同Arts→支配のフォーリナーBusterを再確認し、Quick後とArts連続中に敵AのHP0表示が残り、Foreigner Buster開始時に敵Aが退場して敵Bへ移ることを確認する。
