...
- 状態: No.054「オクタウィアヌス」と絆礼装「父から継いだ名」を実装した。No.025「ドゥズヤールヤー〔騎〕」と絆礼装「雨なき年の星」も実装済み。補助宝具はカード選択・NP消費・OC・効果順を通常宝具と共通化しつつ、攻撃・Hit・ダメージを発生させない。解除可能な弱体状態だけを条件にする特攻、0.1%精度のスター発生率、敵チャージ減少の成功率も共通処理として追加した。No.056「アグリッパ」、No.105「李広」、No.107「サルバドール・ダリ」、共有スキル・状態アイコン素材、同一／別シードの戦闘やり直しは`main`へ統合済み。自動検査後、オクタウィアヌスのユーザー環境での実画面受入を待つ。BattleSession、中断保存形式4・データ1.38.0、直接再開、固定シードリプレイ、カード再配布、6乱数列は維持している。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. オクタウィアヌスのスキル1～3、強化後補助宝具のOC別強化・スター獲得、被ダメージ時スター獲得、絆礼装を実画面で確認する。合格ならPRを`main`へ統合する。
