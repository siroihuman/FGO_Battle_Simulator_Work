...
- フェーズ: 5／初期サーヴァント
- 状態: 共通戦闘・効果・ログ基盤、初期サーヴァント2騎、完全戦闘ループ、保存・再開、礼装選択・開始時適用、アトラス院制服とノーマルカルデア制服の全6スキルを実装済み。次は初期概念礼装の対象・解放状態・Lvを確定する。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. 初期概念礼装の対象、最大解放／未解放、Lvを仕様として確定する（仕様策定担当、推奨モデル: `gpt-5.6-sol`）。具体データ実装は確定後に別作業とする。
