...
- 状態: No.029「ジュリア・ファルネーゼ〔騎〕」と絆礼装「六輪の青百合」を実装した。No.025「ドゥズヤールヤー〔騎〕」、No.054「オクタウィアヌス」、No.055「アウグストゥス」と各絆礼装も登録済み。補助宝具、解除可能な弱体状態のみを条件にする特攻、0.1%精度のスター発生率、敵チャージ減少の成功率、BattleSession、中断保存形式4・データ1.38.0、直接再開、固定シードリプレイ、カード再配布、6乱数列は維持する。自動検査後、ジュリアのユーザー環境での実画面受入を待つ。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

ジュリアの宝具攻撃倍率だけをD-125で訂正した。約15000ダメージの件は保留する。修正後の確認は旧中断保存や再戦ボタンではなく、設定画面から新規戦闘を開始する。
