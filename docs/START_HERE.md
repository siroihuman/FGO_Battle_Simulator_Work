...
- フェーズ: 15／スリップダメージ倍加（仕様確定）
- 状態: 延焼・蝕毒・呪厄の対象、重複、種類別合計後の倍率加算・切捨て、発動時状態の固定、同時精算、解除、控え、ログ、保存・再開・リプレイ、UI接続を仕様として確定。実装コードと具体データは未着手。
- 完成目標: `v1.0`
- 正本: このリポジトリの `main`、文書、実装、テスト。チャット履歴は正本にしない。

## 次の作業

1. 確定済みの延焼・蝕毒・呪厄によるスリップダメージ倍加を共通処理へ実装する（システム実装担当、推奨モデル: `gpt-5.6-sol`、思考レベル: `high`）。`slip_damage`の種類・対応倍率宣言、種類別保留・合計・切捨て、ターン終了ログ、保存・直接再開・固定シードリプレイ、最小UI表示を接続する。初期コンテンツ追加には進まない。
